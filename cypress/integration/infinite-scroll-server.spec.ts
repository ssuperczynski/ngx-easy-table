...
context('Infinite scroll server', () => {
  before(() => {
      cy.server();
      cy.route({
        url: 'https://my-json-server.typicode.com/ssuperczynski/ngx-easy-table/company?_limit=10&_page=1',
        method: 'GET',
        status: 200,
        response: [
          {
  ...
  describe('on scroll', () => {
    it('loads next items', () => {
      const tenthItem = '#table > tbody > cdk-virtual-scroll-viewport > div.cdk-virtual-scroll-content-wrapper > tr:nth-child(10) > td:nth-child(3) > div';
      cy.wait('@firstCall')
        .get(tenthItem)
